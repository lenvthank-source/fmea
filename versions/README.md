...
- `v0.5.7.md` — PFMEA Fragmented Causes Sub-Rows (Export/Import/Report View), Work Element Removal from Report View & Full Dialog Theme Overhaul: unmerged columns 7-23 in PFMEA Excel export, import, and Report View so each cause occupies an independent sub-row with its prevention/detection controls, ratings, AP, and actions; removed Work Element (4M) from Report View for 1:1 Excel parity; modernized top toolbar to segmented pill tabs; overhauled all PFMEA child dialogs (FailureDetailWindow, AddFailureDialog, AddFunctionDialog, MultiAddWorkElementDialog, FailureLinkageModal, PfmeaExcelImportWizard) to Space-Optimized Shadcn Admin theme.
- `v0.5.6.md` — Industrial PFD Flow Diagram Rebuild, Safe Launch Across All Phases, Sticky Unblurred Header, Repository/Actions/Admin Modernization & UI Theme Rule: rebuilt PFD canvas with vector equipment icons (pumps, vessels, reactors, heat exchangers, coils, filters, cyclones), orthogonal piping, and stream flags; enabled Safe Launch across Prototype, Pre-Launch, and Production; sticky zIndex:1350 DocumentHeader unblurred by modal backdrops; eliminated double asterisks; modernized PFD, Repository, Actions, and Admin pages to space-optimized Shadcn Admin theme; authored .agents/rules/ui-theme.md.
